...
/* General params
*/
#define FKO_PROTOCOL_VERSION_3 "3.0.0" /**< The fwknop protocol version */
#define FKO_PROTOCOL_VERSION_4 "4.0.0"
...
#define FKO_DEFAULT_HMAC_MODE    FKO_HMAC_MD5
...
DLL_API int fko_enlink_spa_data_final(fko_ctx_t ctx, const unsigned char * spa_key,
                                 int hmac_type);
...
/**
 * \brief Set FKO deviceid
 *
 * Set the deviceid field of the SPA data.  If deviceid is NULL,
 * libfko will set default value "unknown"

 *
 * \param ctx the FKO context to modify
 * \param deviceid The value to set in the FKO context
 *
 * \return FKO_SUCCESS if successful, returns an error code otherwise.
 */
DLL_API int fko_set_authid(fko_ctx_t ctx, const char * const deviceid);
...
DLL_API int fko_send_spa_data(char *ip, char *ports, char *spaKey, int spaPort, char *devid);
...
/**
 * \brief get deviceid from FKO context
 *
 * Assigns the pointer to the string holding the deviceid associated
 * with the current context to the address deviceid is pointing to.
 *
 * \param ctx The FKO context to access
 * \param Pointer to the pointer to be assigned
 *
 * \return FKO_SUCCESS if successful, returns an error code otherwise
 */
DLL_API int fko_get_deviceid(fko_ctx_t ctx, char **deviceid);
...
DLL_API int fko_get_innerID(fko_ctx_t ctx, char **innerID);


DLL_API int fko_get_outerID(fko_ctx_t ctx, char **outerID);
