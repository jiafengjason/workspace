...
func dnslookup(domain string) {
	...
	var A_records []string
	var AAAA_records []string
	...
	if err == nil {
		for _, ans := range r.Answer {
			...
			if isType {
				...
				A_records = append(A_records, record.A.String())
			}
		}
	}
	...
	if err == nil {
		for _, ans := range r.Answer {
			...
			if isType {
				//fmt.Println("type AAAA:", record.AAAA)
				AAAA_records = append(AAAA_records, record.AAAA.String())
			}
		}
	}
	fmt.Printf("domain:%s A:%v, AAAA:%v\n", domain, A_records, AAAA_records)
}
...
func lookup(domain string) {
	...
	if err == nil {
		for _, line := range bytes.Split(out, []byte("\n")) {
			if strings.HasPrefix(string(line),"Address:") && !strings.Contains(string(line), "#")	{
				items := strings.Split(string(line), `:`)
				println(strings.TrimSpace(items[1]))
			}
		}
	}
}
