    ...
    if json then
        ...
        elseif is_tab(body) then
            ...
            for k, v in pairs(body) do
                -- 解决url encode 错误的问题，tostring不行，ngx.escape_uri也可以，但是必须依赖nginx
                -- param[#param + 1] = format("%s=%s", tostring(k), tostring(v))
                -- param[#param + 1] = format("%s=%s", ngx.escape_uri(k), ngx.escape_uri(v))
                param[#param + 1] = format("%s=%s", urlEncode(k), urlEncode(v))
            end
