#!/bin/bash
# 模拟攻击：连接加密货币挖矿池 - 触发EDR检测规则

# 挖矿池域名列表（从规则中提取的关键域名）
POOLS=(
    "minexmr.com"
    "xmr.nanopool.org"
    "pool.supportxmr.com"
    "monero.hashvault.pro"
    "xmr.pool.minergate.com"
    "monero.herominers.com"
    "pool.minexmr.com"
    "xmr.2miners.com"
    "xmrpool.net"
    "supportxmr.com"
    "nanopool.org"
    "minergate.com"
    "moneropool.com"
    "monerpool.org"
    "linux.monerpool.org"
    "xmrpool.eu"
    "xmrpool.de"
    "xmrpool.me"
    "xmrpool.xyz"
    "donate.xmrig.com"
    "randomx.xmrig.com"
    "fee.xmrig.com"
    "donate.ssl.xmrig.com"
    "donate.v2.xmrig.com"
    "xmrigcc.graef.in"
    "donate.graef.in"
    "donate2.graef.in"
    "nbminer.com"
    "lhr.nbminer.com"
    "lhr3.nbminer.com"
    "dl.nbminer.com"
    "herominers.com"
    "miner.rocks"
    "minerrocks.com"
    "miningpoolhub.com"
    "suprnova.cc"
    "dwarfpool.com"
    "multipool.us"
    "poolmining.org"
    "prohash.net"
    "siamining.com"
    "ratchetmining.com"
    "hellominer.com"
    "minercircle.com"
    "miners.pro"
    "monero.miners.pro"
    "monerohash.com"
    "monerogb.com"
    "moneroocean.stream"
    "monerocean.stream"
    "monerorx.com"
    "moriaxmr.com"
    "mine.moneropool.com"
    "mine.c3pool.com"
    "mine.zpool.ca"
    "mine.ppxxmr.com"
    "mine1.ppxxmr.com"
    "minemonero.gq"
    "miner.ppxxmr.com"
    "minerpool.pw"
    "eu.minerpool.pw"
    "minerxmr.ru"
    "minexmr.cn"
    "mining-help.ru"
    "mixpools.org"
    "moner.monerpool.org"
    "moner1min.monerpool.org"
    "xmr1min.monerpool.org"
    "monero-master.crypto-pool.fr"
    "monero.crypto-pool.fr"
    "xmr.crypto-pool.fr"
    "xmr.crypto-pool.info"
    "crypto-pool.fr"
    "crypto-pool.info"
    "monero.lindon-pool.win"
    "monero.riefly.id"
    "monero.us.to"
    "xmr.f2pool.com"
    "xmr.hashcity.org"
    "xmr.hex7e4.ru"
    "xmr.ip28.net"
    "xmr.monerpool.org"
    "xmr.mypool.online"
    "xmr.pool.gntl.co.uk"
    "xmr.poolto.be"
    "xmr.ppxxmr.com"
    "xmr.prohash.net"
    "xmr.simka.pw"
    "xmr.somec.cc"
    "xmr.suprnova.cc"
    "xmr.usa-138.com"
    "xmr.vip.pool.minergate.com"
    "xmrfast.com"
    "xmrminer.cc"
    "xmrf.520fjh.org"
    "xmrf.fjhan.club"
    "xx11m.monerpool.org"
    "xx11mv2.monerpool.org"
    "xxx.hex7e4.ru"
    "pool.4i7i.com"
    "pool.armornetwork.org"
    "pool.cortins.tk"
    "pool.gntl.co.uk"
    "pool.hashvault.pro"
    "pool.minergate.com"
    "pool.monero.hashvault.pro"
    "pool.ppxxmr.com"
    "pool.somec.cc"
    "pool.support"
    "pool.usa-138.com"
    "pool.xmr.pt"
    "pool.xmrfast.com"
    "pool2.armornetwork.org"
    "poolchange.ppxxmr.com"
    "pooldd.com"
    "poolto.be"
    "ppxvip1.ppxxmr.com"
    "ppxxmr.com"
    "xmr-asia1.nanopool.org"
    "xmr-au1.nanopool.org"
    "xmr-eu1.nanopool.org"
    "xmr-eu2.nanopool.org"
    "xmr-jp1.nanopool.org"
    "xmr-us-east1.nanopool.org"
    "xmr-us-west1.nanopool.org"
    "xmr-us.suprnova.cc"
    "xmr-usa.dwarfpool.com"
    "xmr.5b6b7b.ru"
    "xmr.alimabi.cn"
    "xmr.bohemianpool.com"
    "alimabi.cn"
    "ap.luckpool.net"
    "eu.luckpool.net"
    "na.luckpool.net"
    "node3.luckpool.net"
    "luckpool.net"
    "bcn.pool.minergate.com"
    "bcn.vip.pool.minergate.com"
    "fcn-xmr.pool.minergate.com"
    "mro.pool.minergate.com"
    "xmc.pool.minergate.com"
    "xmo.pool.minergate.com"
    "ca.minexmr.com"
    "de.minexmr.com"
    "fr.minexmr.com"
    "sg.minexmr.com"
    "us-west.minexmr.com"
    "ca.monero.herominers.com"
    "lokiturtle.herominers.com"
    "cbd.monerpool.org"
    "cbdv2.monerpool.org"
    "daili01.monerpool.org"
    "xiazai.monerpool.org"
    "xiazai1.monerpool.org"
    "huadong1-aeon.ppxxmr.com"
    "jw-js1.ppxxmr.com"
    "koto-pool.work"
    "masari.miner.rocks"
    "sumokoin.minerrocks.com"
    "turtle.miner.rocks"
    "trtl.cnpool.cc"
    "trtl.pool.mine2gether.com"
    "bohemianpool.com"
    "cryptmonero.com"
    "cryptonight-hub.miningpoolhub.com"
    "d1pool.ddns.net"
    "d5pool.us"
    "emercoin.com"
    "emercoin.net"
    "emergate.net"
    "ethereumpool.co"
    "iwanttoearn.money"
    "pangolinminer.comgandalph3000.com"
    "r.twotouchauthentication.online"
    "seed.emercoin.com"
    "seed.emercoin.net"
    "seed.emergate.net"
    "seed1.joulecoin.org"
    "seed2.joulecoin.org"
    "seed3.joulecoin.org"
    "seed4.joulecoin.org"
    "seed5.joulecoin.org"
    "seed6.joulecoin.org"
    "seed7.joulecoin.org"
    "seed8.joulecoin.org"
    "sheepman.mine.bz"
    "teracycle.net"
    "usxmrpool.com"
    "viaxmr.com"
    "webservicepag.webhop.net"
    "zarabotaibitok.ru"
    "zer0day.ru"
)

# 快速批量连接所有挖矿池域名
echo "开始连接挖矿池域名..."
for pool in "${POOLS[@]}"; do
    # 方法1: DNS查询（最快，触发检测）
    timeout 0.2 host "$pool" >/dev/null 2>&1 || \
    # 方法2: HTTP连接
    timeout 0.2 curl -s --connect-timeout 0.1 "http://$pool" >/dev/null 2>&1 || \
    # 方法3: HTTPS连接
    timeout 0.2 curl -s --connect-timeout 0.1 "https://$pool" >/dev/null 2>&1
done

echo "攻击模拟完成！"


